...
Data on [Energy Certificates](https://epc.opendatacommunities.org/) of building in England and Wales are provided by the Department for Levelling Up, Housing & Communities. These certificates are a non-random sample of UK housing, and contain information such as property energy efficiency, CO~2~ emissions, energy costs, and improvement recommendations. In total, there are 93 variables available on a wide range of metrics (see glossary of terms at <https://epc.opendatacommunities.org/docs/guidance> for more details).

This project aims to determine whether some local authorities are better than others in terms of reducing the carbon footprint of properties in their area. However, the age and size of house in each area must be considered due to older and larger houses having a larger carbon footprint. Therefore I will be performing aggregate data analysis where the unit of analysis is a local authority. The average carbon footprint of a house in a given area will be considered, but will be conditional on whether it was build before or after 1930, and the number of heated rooms. I will then be able to create a model with fixed effects for the number of houses built pre-1930, post-1930 and number of heated rooms:
...
This plot can be used to assess the each local authority's performance, by reading off the effect size value. For example, if an authority had a value of -0.89, its baseline carbon emissions are 0.89 tonnes/year lower than average. This number can also be plugged into the model as the $\gamma_j$ value to give the average carbon emissions for a specific house type, which using the fixed effects results from the previous section, would be the following:

$$Y_{ji} = 2.41 + 2.98 x_{j}^{(\text{old})} + \sum_{r=1}^{10} 1.05 x_{j}^{(r)} + \gamma_j$$

## Additional Credit - Regression Diagnostic Plots

To extend this work slightly further, I decided to create regression diagnostic plots for the model, which required installing `matplotlib`. The plots can be found below:
...
The final plot is again used to assess the homoscedasticity assumption by plotting the square root of the absolute standardised residuals against the fitted values. As suggested by the residuals vs fitted plot, there appears to be heteroscedasticity, as the data points do not fall along a straight horizontal line, but instead form an almost W-shape.

Overall, the plots indicate that there are multiple violations of the assumptions of the linear model. This suggests that in future, the model should be modified significantly. This could include transforming the outcome variable (CO~2~ emissions) using a logarithm, using a non-linear model such as a GAMM, or adding in non-linear terms.

## Record of AI Use

| **Date** | **AI tool used** | **Purpose** | **Prompt** | **Hyperlink to output (where possible)** | **Section of work used for** |
|------------|------------|------------|------------|------------|------------|
| 09/12/2025 | ChatGPT | To help extend the regression pipeline by including diagnostics | "How would I extend this pipeline to include regression diagnostic plots?: \[copied contents of fit_model.py\]" |  | Additional Credit |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
