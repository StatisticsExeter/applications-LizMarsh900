...
Reducing the carbon footprint of UK housing is an increasingly important endeavor as global temperatures rise. In attempting to achieve this, it's important to develop a strategy that is tailored to property types, because older and newer houses require different approches. Therefore, this project will aim to establish whether it's possible to determine the age of a house based on several recorded energy metrics. These metrics are:
...
The QDA classifier achieved an overall accuracy of 0.621 and was slightly better at distinguishing post-1930s homes (F1 = 0.681) then pre-1930s homes (F1 = 0.534). However, the classifier performed worse than the LDA classifier in every domain (F1 scores, accuracy), showing that the LDA classifier is the better model. This also suggests that the underlying separation between older and newer homes is linear in nature.
...
Overall, using the LDA classifier it would be possible for a local authority to determine the age of a house with a 74% accuracy rate, without visiting it. This will help inform the targets set for reducing carbon footprint.
